...
window.onload = function(){
	...
	%>
	if(oSex1.value == "<%=stu1.getSex() %>"){ //为性别单选框赋值
		oSex1.checked = true;
	}
	else{
		oSex2.checked = true;
	...
	oPerovince.value = "<%=stu1.getPerovince() %>";//为籍贯下拉框赋值
	
	if(oInterest1.value == "<%=stu1.getInterests() %>"){ //为兴趣单选框赋值
		oInterest1.checked = true;
	}
	else if(oInterest2.value == "<%=stu1.getInterests() %>"){
		oInterest2.checked = true;
	}
	else if(oInterest3.value == "<%=stu1.getInterests() %>"){
		oInterest3.checked = true;
	}
	else if(oInterest4.value == "<%=stu1.getInterests() %>"){
		oInterest4.checked = true;
	}
	else if(oInterest5.value == "<%=stu1.getInterests() %>"){
...
<%
	...
	String student_id = stu.getStu_id();
%>
<br>

<a href="student_info.jsp">返回个人中心</a>&nbsp;&nbsp;<a href="logout.jsp">注销</a>
<div>
	<form action="StudentUpdateServlet" method="post" name="form1">
		<p>
			<span class="word">学号：<span class="star">*</span></span>
			<input class="input" type="text" name="student_id" value=<%=student_id %> required/>
		</p>
		
		<p>
			<span class="word">姓名：<span class="star">*</span></span>
			<input class="input" type="text" name="name" value=<%=stu.getName() %> required/>
		</p>
		
		<p>
			<span class="word">身份证号：<span class="star">*</span></span>
			<input class="input" type="text" name="card_id" value=<%=stu.getCard_id() %> required/>
		</p>
		
		<p>
			<span class="word">班级：<span class="star">*</span></span>
			<input class="input" type="text" name="stu_class" value=<%=stu.getStu_class() %> required/>
		</p>
		...
		<p>
			...
			<input class="input" type="text" name="password" value=<%=stu.getPassword() %> required/>
		</p>
		...
		<p>
			...
			<input class="input" type="text" name="address" value=<%=stu.getAddress() %> required/>
		</p>
		
		<p>
			<span class="word">手机号：<span class="star">*</span></span>
			<input class="input" type="text" name="tel" value=<%=stu.getTel() %> required/>
		</p>
